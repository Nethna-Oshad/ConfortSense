...
export default function AdminSettingsPage() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="Admin Settings" backHref="/admin/overview" backLabel="Back to Overview" />
      <div className="max-w-xl rounded-2xl border border-[#294467]/60 bg-[#0E1C30] p-5">
        <p className="text-sm text-[#7F93B3]">Facility profile</p>
        <p className="mt-1 text-lg font-semibold">{user?.name || "Campus Admin"}</p>
        <p className="text-sm text-[#5A6C8A]">{user?.email || "admin@university.edu"}</p>
      </div>
      <button
        onClick={() => {
          clearAuth();
